...
input_1 = ["Иванов Иван Иванович", "Москва", "Специалист", "верстальщик"]

def to_lower(str_list):
  return [x.lower() for x in str_list]

input_lower = ["иванов иван иванович", "москва", "специалист", "верстальщик"]



def process_input_func(input):
    ...
    unknown_input = [x for x in input_lower if
                     x not in enterprise_branch_lower and x not in grade_lower and x not in job_title_lower and x not in technologies_lower]
    unknown_input
    ...
    if "имя" not in input_types:
        ...
        raise IOError("Ошибка вы не ввели имя работника!")
    if "филиал предприятия" not in input_types:
        print("Ошибка вы не ввели филиал предприятия!")
        raise IOError("Ошибка вы не ввели филиал предприятия!")
    if "грейд" not in input_types:
        print("Ошибка вы не ввели грейд работника!")
        raise IOError("Ошибка вы не ввели грейд работника!")
    if "название должности" not in input_types:
        print("Ошибка вы не ввели название должности!")
        raise IOError("Ошибка вы не ввели название должности!")

    if input_types.count("имя") > 1:
        print("Ошибка вы ввели более одного имени!")
        raise IOError("Ошибка вы ввели более одного имени!")
    if input_types.count("филиал предприятия") > 1:
        print("Ошибка вы ввели более одного филиал предприятия!")
        raise IOError("Ошибка вы ввели более одного филиал предприятия!")
    if input_types.count("грейд") > 1:
        print("Ошибка вы ввели более одного грейд!")
        raise IOError("Ошибка вы ввели более одного грейда!")
    if input_types.count("название должности") > 1:
        print("Ошибка ввели более одного название должности!")
        raise IOError("Ошибка ввели более одного название должности!")

    if input_types.count("NaN") > 0:
        print("Есть не идентифицированный ввод!")
        print([input[i] for i in range(len(input_types)) if input_types[i] == "NaN"])
        raise IOError("Есть не идентифицированный ввод!")
